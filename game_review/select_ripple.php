<?php
        ...
        $sql = "SELECT * from `game_review_ripples` where `info_num` = $num order by `num` desc";
        $result = mysqli_query($dbcon, $sql) or die("review ripples list select error1 : " . mysqli_error($dbcon));
        ...
        for ($i = $start; $i < $start + SCALE && $i < $total_record; $i++) {
            $sql = "SELECT * from `game_review_ripples` where `info_num` = $num order by num desc";
            $result4 = mysqli_query($dbcon, $sql) or die("review ripple select error1 : ".mysqli_error($dbcon));
