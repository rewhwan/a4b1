                ...
                for ($i = $start; $i < $start + SCALE && $i < $total_record; $i++) {
                    ...
                    if ($mode == "search") {
                        switch ($search) {
                            case "name":
                            case "grade":
                                $sql1 = "SELECT * from `game_info`  where $search like '%$search_word%' order by `num` desc";
                                break;
                            case "genre":
                            case "platform":
                                $sql1 = "SELECT * from `game_info` a left join game_info_$search b on a.num=b.info_num where $search like '%$search_word%' order by a.num desc";
                                break;
                            default:
